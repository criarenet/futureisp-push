...
el=el.parentNode}
return null}
function Slideout(options){options=options||{};this._startOffsetX=0;this._currentOffsetX=0;this._opening=!1;this._moved=!1;this._opened=!1;this._preventOpen=!1;this.panel=options.panel;this.menu=options.menu;this._touch=options.touch===undefined?!0:options.touch&&!0;this._side=options.side||'left';this._easing=options.fx|| options.easing||'ease';this._duration=parseInt(options.duration,10)||300;this._tolerance=parseInt(options.tolerance,10)||70;this._padding=this._translateTo=parseInt(options.padding,10)||256;this._orientation=this._side==='right'?-1:1;this._translateTo*=this._orientation;if(!this.panel.classList.contains('slideout-panel')){this.panel.classList.add('slideout-panel')}
...
this.emit('beforeclose');this._setTransition();this._translateXTo(0);this._opened=!1;setTimeout(function(){html.classList.remove('slideout-open');self.panel.style.transition=self.panel.style['-webkit-transition']=self.panel.style[prefix+'transform']=self.panel.style.transform='';self.emit('close')},this._duration+50);return this};Slideout.prototype.toggle=function(){return this.isOpen()?this.close():this.open()};Slideout.prototype.isOpen=function(){return this._opened};Slideout.prototype._translateXTo=function(translateX){this._currentOffsetX=translateX;this.panel.style[prefix+'transform']=this.panel.style.transform='translateX('+translateX+'px)';return this};Slideout.prototype._setTransition=function(){this.panel.style[prefix+'transition']=this.panel.style.transition=prefix+'transform '+this._duration+'ms '+this._easing;return this};Slideout.prototype._initTouchEvents=function(){var self=this;this._onScrollFn=decouple(doc,'scroll',function(){if(!self._moved){clearTimeout(scrollTimeout);scrolling=!0;scrollTimeout=setTimeout(function(){scrolling=!1},250)}});this._preventMove=function(eve){if(self._moved){eve.preventDefault()}};doc.addEventListener(touch.move,this._preventMove);this._resetTouchFn=function(eve){if(typeof eve.touches==='undefined'){return}
self._moved=!1;self._opening=!1;self._startOffsetX=eve.touches[0].pageX;if(self._startOffsetX>60){return}
self._preventOpen=(!self._touch||(!self.isOpen()&&self.menu.clientWidth!==0))};this.panel.addEventListener(touch.start,this._resetTouchFn);this._onTouchCancelFn=function(){self._moved=!1;self._opening=!1};this.panel.addEventListener('touchcancel',this._onTouchCancelFn);this._onTouchEndFn=function(){if(self._moved){self.emit('translateend');(self._opening&&Math.abs(self._currentOffsetX)>self._tolerance)?self.open():self.close()}
self._moved=!1};this.panel.addEventListener(touch.end,this._onTouchEndFn);this._onTouchMoveFn=function(eve){if(self._startOffsetX>20&&!self._opened){return}
if(scrolling||self._preventOpen||typeof eve.touches==='undefined'||hasIgnoredElements(eve.target)){return}
